...
file_name = uigetfile; %Pour sélectionner des fichiers depuis l'ordinateur
...
handles.y1=y; %On passe les données de la musique à la sub-class handle de la fct! (sample data) Data non modifié !
...
handles.Fs2=Fs;
guidata(hObject,handles); %On recopie ces objets contenant les données dans le dossier du GUI! (en commun)
...
y=handles.y1; %On récupère le fichier Wav import data et il peut avoir un effet dessus !
Fs=handles.Fs; %Idem pour le rate du fichier wav
handles.player=audioplayer(y,Fs); %On stocke en mémoire dans le sub class handle l'audio_player
play(handles.player); %Fonction permettant de jouer une musique
guidata(hObject,handles); %On renvoit les nouvelles données à l'ensemble du GUI! Pour pouvoir les utiliser ailleurs!
